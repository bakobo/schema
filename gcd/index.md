...
* The `c_upto` field creates a ceiling for the financial stakes of the delegated action; that is, delegate can act only in contexts where a financial value < N is at stake. Value is a string that contains a number followed by a space, then units: an ISO 4217 currency symbol, by a three-letter cryptocurrency abbreviation, or by another brief single-token symbol with obvious meaning: "25 CHF", "0.3 BTC", "4 OZ-XAU".
